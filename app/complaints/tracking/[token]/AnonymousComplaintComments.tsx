...
interface AnonymousComplaintCommentsProps {
  token: string;
}

export function AnonymousComplaintComments({ token }: AnonymousComplaintCommentsProps) {
  const [comment, setComment] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);

  const { addComment } = useAnonymousComplaintComments();
  const { data: comments = [], isLoading: isLoadingComments } = useAnonymousComplaintCommentsList(token);
