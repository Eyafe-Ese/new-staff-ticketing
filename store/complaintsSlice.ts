...
export const fetchComplaints = createAsyncThunk(
  ...
  async (params: {
    ...
    try {
      ...
      const response = await api.get('/complaints/my-complaints', { params: queryParams });
...
export const addAttachments = createAsyncThunk(
  ...
  async ({ 
    ...
    try {
      ...
      const response = await api.post(`/complaints/${complaintId}/attachments/multiple`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      return response.data.data;
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || 'Failed to add attachments');
    }
  }
);
    ...
    builder
      ...
      .addCase(addComment.fulfilled, (state, action) => {
        ...
        if (state.selectedComplaint) {
          state.selectedComplaint.comments = state.selectedComplaint.comments || [];
      ...
      .addCase(addAttachments.fulfilled, (state, action) => {
        ...
        if (state.selectedComplaint) {
          state.selectedComplaint.attachments = state.selectedComplaint.attachments || [];
      ...
      .addCase(deleteAttachment.fulfilled, (state, action) => {
        ...
        state.attachments.items = state.attachments.items.filter(
          item => !(item.id === attachmentId && state.selectedComplaint?.id === complaintId)
        );
        // Remove from selected complaint if it exists
        if (state.selectedComplaint?.id === complaintId) {
          state.selectedComplaint.attachments = state.selectedComplaint.attachments?.filter(
...
export const selectSelectedComplaint = (state: RootState) => state.complaints.selectedComplaint;
export const selectComplaintsStatus = (state: RootState) => state.complaints.status;
export const selectComplaintsError = (state: RootState) => state.complaints.error;
export const selectComplaintsFilters = (state: RootState) => state.complaints.filters;
export const selectComplaintsPagination = (state: RootState) => state.complaints.pagination;
export const selectComplaintsComments = (state: RootState) => state.complaints.comments;
export const selectComplaintsAttachments = (state: RootState) => state.complaints.attachments;
